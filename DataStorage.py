...
def save(input, camera=""):
	#code to store frame as an unique image here
	newImage1 = np.copy(input)
	dimen = np.shape(newImage1)
	now = datetime.datetime.now()
	tempfileName1 = "data/" + str(now.year)+"_"+str(now.month)+"_"+str(now.day)+"_"+str(now.hour)+"_"+str(now.minute)+"_"+str(now.second)+camera + ".png"
	cv2.imwrite(tempfileName1, newImage1)

	#code to store information in a .txt file
	f= open("data/" + str(now.year)+"_"+str(now.month)+"_"+str(now.day)+"_"+str(now.hour)+"_"+str(now.minute)+"_"+str(now.second)+camera + ".txt","w+")
	f.write(str(now))
	f.write("\n")
	...
	count = np.shape(centroids)
		...
		#run a loop to write a line in .txt for each distinct strawberry
	if(count[0] == 0):
