...
class DataStorage:
	...
	def save(input, camera=""):
		...
		tempfileName1 = str(now.year)+"_"+str(now.month)+"_"+str(now.day)+"_"+str(now.hour)+"_"+str(now.minute)+"_"+str(now.second)+camera + ".png"
		cv2.imwrite(tempfileName1, newImage1)
		
		#code to store information in a .txt file
		f= open(str(now.year)+"_"+str(now.month)+"_"+str(now.day)+"_"+str(now.hour)+"_"+str(now.minute)+"_"+str(now.second)+camera + ".txt","w+")
		f.write(str(now))
		f.write("\n")
